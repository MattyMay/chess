...
public class GamePlayer extends Thread implements Serializable {
     ...
     * 
     * @param gcb
     *            - an instance of the Interface_ConnectionBroken interface to
     *            be called when the Socket disconnects.
     */
     ...
     * reading the ObjectOutputStream from the GameControl. Every MyGameOutput
     * read from the ObjectOutputStream is passed on by calling the
     * receivedMessage routine. Your Networked Game should not be accessing the
     * run routine directly. It has to be public to properly run as a thread.
     */
    public void run() {
    ...
    void joinGame() {
        ...
        try {
            gameSocket = new Socket(gameControl.getIpAddress(), gameControl.getPortNum());
            if (gameSocket == null)
     ...
     *            Object received from the GameControl
     */
